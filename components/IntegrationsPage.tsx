...
export default function IntegrationsPage({ onBack }: { onBack: () => void }) {
  ...
  const confirmJanusIntegration = async () => {
    ...
    const JANUS_SYNC_URL = isLocal 
      ...
      : (process.env.NEXT_PUBLIC_JANUS_URL || "http://localhost:3001/api/janus/v1/sync");
  ...
  const handleOpenJanus = () => {
    ...
    const JANUS_BASE_URL = isLocal 
      ? "http://localhost:3001" 
      : (process.env.NEXT_PUBLIC_JANUS_URL || "http://localhost:3001").replace('/api/janus/v1/sync', '').replace(/\/$/, "");
