...
export default function MeetingExternalPage() {
    ...
    const handleAddTopic = async (sectionId: number) => {
        ...
        if (!insertErr && data) {
            setSections((prev) =>
                prev.map((s) =>
                    s.id === sectionId
                        ? { ...s, topics: [...s.topics, { id: data.id, title: data.title, description: data.description, section_id: data.section_id, order_index: data.order_index }] }
                        : s
                )
            )
            setNewTopicTitle("")
    ...
    const saveEditTopic = async (topicId: number, sectionId: number | null) => {
        ...
        const { error: updateErr } = await supabase
            .from("topics")
            .update({ title: editTopicTitle.trim(), description: editTopicDesc.trim() || null })
            .eq("id", topicId)
                    ...
                    s.id === sectionId
                        ? {
                            ...
                            topics: s.topics.map((t) =>
                                t.id === topicId
                                    ? { ...t, title: editTopicTitle.trim(), description: editTopicDesc.trim() || null }
                                    : t
                            ),
                        }
                        : s
