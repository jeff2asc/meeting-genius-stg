...
export async function POST(request: NextRequest) {
  try {
    ...
    // All parameter types can be company-scoped. meeting_type and voting_type
    // previously forced company_id = null, which caused duplicate rows and broken
    // linked_voting_type lookups. They now respect the company_id from the client.
    const effectiveCompanyId = company_id || null
    const supabase = createAdminClient()
